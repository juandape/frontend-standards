...
console.log(
  `[frontend-standards-checker] Directorio del paquete: ${packageRoot}`
);
console.log(
  `[frontend-standards-checker] Directorio del proyecto: ${projectRoot}`
);
  ...
  if (!fs.existsSync(dest)) {
    if (fs.existsSync(src)) {
      try {
        ...
        console.error(
          `[frontend-standards-checker] ❌ Error copiando ${file}:`,
          error.message
        );
      }
    } else {
      console.warn(
        `[frontend-standards-checker] ⚠️  Archivo no encontrado: ${src}`
      );
    }
  } else {
